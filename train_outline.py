"""Fine-tune T5-small: prompt → EtriCA-style event-sequence outline.
...
Usage:
    ...
    python train_outline.py --epochs 3

    # WritingPrompts, one epoch at a time (resumable across Colab sessions)
    python train_outline.py --data wp --epochs 1
    python train_outline.py --data wp --epochs 1 --resume   # next session
    python train_outline.py --data wp --epochs 1 --resume   # next session
...
DATA_CONFIG = {
    "roc": {
        "train": "data/processed/outline_train.jsonl",
        "val":   "data/processed/outline_val.jsonl",
        "out":   "models/t5_outline",
        "batch": 16,
    },
    "wp": {
        "train": "data/processed/wp_outline_train.jsonl",
        "val":   "data/processed/wp_outline_val.jsonl",
        "out":   "models/t5_outline_wp",
        "batch": 8,
    },
}
...
def train(args: argparse.Namespace) -> None:
    ...
    if args.resume and state_file.exists():
        ...
        total_steps   = None  # set after dataloaders are built

    # ── Data ────────────────────────────────────────────────────────────────
    train_loader = DataLoader(
        OutlineDataset(cfg["train"], tokenizer),
        batch_size=cfg["batch"], shuffle=True, num_workers=0,
    )
    val_loader = DataLoader(
        OutlineDataset(cfg["val"], tokenizer),
